...
public class GroceryCreation {
    ...
    static List<Grocery> groceries = new ArrayList<>();
    ...
    public static void createListOfVegetable() {
        ...
        while(x < 25) {
            ...
            vegetable = vegetable.replace(' ', '_');
            item.name = vegetable;
            item.type = "vegetable";
            item.price = df.format(Math.random() * (6 - 1 + 1 )+ 1);
            item.quantity = 1000;
    ...
    public static void createListOfFruits() {
        ...
        while(x < 25) {
            ...
            item.name = fruit;
            item.type = "fruit";
            item.price = df.format(Math.random() * (8 - 3 + 3)+ 1);
            item.quantity = 1000;
            if(!groceries.isEmpty()) {
                for(Grocery g : groceries) {
